    ...
    struct instance_naxxramas_InstanceMapScript : public InstanceScript
    {
        ...
        void OnPlayerKilled(Player *player) 
        {
            if(IsEncounterInProgress())
                PlayerDeathCount++;
            if(GetBossState(DATA_HEIGAN) == IN_PROGRESS)
                    ...
                    switch(state)
                    {
                        case NOT_STARTED:
                            if (GameObject* AracPortal = instance->GetGameObject(AracPortalGUID))
                                AracPortal->SetPhaseMask(2,true);
                            break;
                        case DONE:
                        {
                            if (GameObject* AracPortal = instance->GetGameObject(AracPortalGUID))
                                AracPortal->SetPhaseMask(1,true);
                            DoTaunt();
                        }
                            break;
                        default:
                    ...
                    switch(state)
                    {
                        case NOT_STARTED:
                            if (GameObject* PlagPortal = instance->GetGameObject(PlagPortalGUID))
                                PlagPortal->SetPhaseMask(2,true);
                            break;
                        case DONE:
                        {
                            if (GameObject* PlagPortal = instance->GetGameObject(PlagPortalGUID))
                                PlagPortal->SetPhaseMask(1,true);
                            DoTaunt();
                        }
                            break;
                        default:
                    ...
                    switch(state)
                    {
                        case NOT_STARTED:
                            if (GameObject* ConsPortal = instance->GetGameObject(ConsPortalGUID))
                                ConsPortal->SetPhaseMask(2,true);
                            break;
                        case DONE:
                        {
                            if (GameObject* ConsPortal = instance->GetGameObject(ConsPortalGUID))
                                ConsPortal->SetPhaseMask(1,true);
                            DoTaunt();
                        }
                            break;
                        default:
                    ...
                    switch(state)
                    {
                        case NOT_STARTED:
                            if (GameObject* MiliPortal = instance->GetGameObject(MiliPortalGUID))
                                MiliPortal->SetPhaseMask(2,true);
                            break;
                        case DONE:
                        {
                            ...
                            if (GameObject* MiliPortal = instance->GetGameObject(MiliPortalGUID))
                                MiliPortal->SetPhaseMask(1,true);
                            DoTaunt();
                        }
                            break;
                        default:
    ...
    struct mr_bigglesworth_npcAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer)
        {
            if (instance->GetBossState(DATA_KELTHUZAD) == DONE)
                return;
            if (Creature* KelThuzad = Unit::GetCreature(*me, instance ? instance->GetData64(DATA_KELTHUZAD_SPEAKER) : 0))
            {
...
{
    public:
        GOHello_go_naxxramas_portal() : GameObjectScript("go_naxxramas_portal") {}
